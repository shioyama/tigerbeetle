...
pub fn run(allocator: std.mem.Allocator, args: Args) !void {
    ...
    const superblock_buffer = try allocator.alignedAlloc(
        u8,
        constants.sector_size,
        vsr.superblock.superblock_zone_size,
    );
    defer allocator.free(superblock_buffer);

    try sio.read_buffer(superblock_buffer, .superblock, 0);

    var superblock_headers: [constants.superblock_copies]SuperBlockHeader = undefined;
    for (&superblock_headers, 0..) |*header, copy| {
        const offset = @as(u64, copy) * vsr.superblock.superblock_copy_size;
        header.* = @as(*const SuperBlockHeader, @alignCast(
            std.mem.bytesAsValue(
                SuperBlockHeader,
                superblock_buffer[offset..][0..@sizeOf(SuperBlockHeader)],
            ),
        )).*;
    }

    var quorums = SuperBlockQuorums{};
    const quorum = quorums.working(&superblock_headers, .open) catch |err| {
        log.err("failed to resolve superblock quorum: {}", .{err});
        return error.SuperBlockQuorumFailed;
    };
    if (!quorum.valid) {
        log.err("superblock quorum is invalid", .{});
        return error.SuperBlockQuorumInvalid;
    }

    const old_superblock = quorum.header;
    ...
    if (args.drain_pipeline) {
        ...
        const parsed_addresses = vsr.parse_addresses(
            args.addresses.?,
            &address_buf,
        ) catch {
            log.err("invalid --addresses", .{});
            return error.InvalidAddresses;
        };
    ...
    if (args.sync_ops and checkpoint_op == 0) {
        log.err("--sync-ops requires a non-root checkpoint (checkpoint_op=0)", .{});
        return error.SyncOpsInvalidForRootCheckpoint;
    }
...
fn create_tmp_datafile(allocator: std.mem.Allocator) !struct {
    ...
    const tmp_path: [:0]const u8 = "/tmp/tb-snapshot-test.tigerbeetle";
...
fn cleanup_tmp_datafile(allocator: std.mem.Allocator, ctx: anytype) void {
    ctx.storage.deinit();
...
test "integration: format, rewrite superblock, verify quorum" {
    ...
    // Resolve quorum on original.
    var quorums = SuperBlockQuorums{};
    const quorum = try quorums.working(&superblock_headers, .open);
    try std.testing.expect(quorum.valid);
    const old = quorum.header;
    ...
    const wal_result = scan_wal(
        std.mem.bytesAsSlice(
            vsr.Header.Prepare,
            // This won't work — we need to read the WAL
            superblock_buffer[0..constants.journal_size_headers],
        ),
        old.cluster,
        old.vsr_state.view,
        old.vsr_state.commit_max,
    ...
    // Actually, we need to read the WAL separately.
    const wal_headers_buffer = try allocator.alignedAlloc(
        u8,
        constants.sector_size,
        constants.journal_size_headers,
    ...
    var new_headers: [constants.superblock_copies]SuperBlockHeader = undefined;
    for (&new_headers, 0..) |*header, copy| {
        const offset = @as(u64, copy) * vsr.superblock.superblock_copy_size;
        header.* = @as(*const SuperBlockHeader, @alignCast(
            std.mem.bytesAsValue(
                SuperBlockHeader,
    ...
    // All copies should be valid and agree.
    var quorums2 = SuperBlockQuorums{};
    const quorum2 = try quorums2.working(&new_headers, .open);
    try std.testing.expect(quorum2.valid);

    const result = quorum2.header;
    try std.testing.expectEqual(old.sequence + 1, result.sequence);
...
test "integration: format, clear WAL, verify all slots reserved" {
    ...
    // Read original superblock for cluster ID.
    const superblock_buffer = try allocator.alignedAlloc(
        u8,
        constants.sector_size,
        vsr.superblock.superblock_zone_size,
    ...
    for (0..constants.journal_slot_count) |slot| {
        const header: vsr.Header.Prepare = if (slot == 0)
            vsr.Header.Prepare.root(old.cluster)
        else
...
fn rewrite_and_open(allocator: std.mem.Allocator, ctx: anytype, args: Args) !SuperBlockHeader {
