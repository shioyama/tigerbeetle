...
5. **`fork-versions.txt` in sync.** The manifest lists the four most recent `X.Y.Z-shopify1` tags reachable from `HEAD^`. CI uses it to stage fork binaries for vortex's multi-version slots.
